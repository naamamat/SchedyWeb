...
function TableShiftWorker({ schedule, days, filterId, sortOrder, selectedPosition, refreshSchedule }) {
  const { user } = useUserContext();


const filterWorkers = (workers) => {
  if (!filterId) return workers; // Show all workers if no filter
  return workers.filter(worker => 
    worker.id.toLowerCase() === filterId.toLowerCase()
  );
};
...
export default TableShiftWorker;
